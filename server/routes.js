...
export async function registerRoutes(app) {
    ...
    app.post("/api/auth/register", async (req, res) => {
        ...
        if (role === "driver")
            return res.status(403).json({ message: "Les comptes livreurs sont crees uniquement par l'administration" });
        if (role === "admin")
            return res.status(403).json({ message: "Acces interdit" });
    ...
    app.get("/api/restaurants/:id", async (req, res) => {
        ...
        if (!r)
            return res.status(404).json({ message: "Restaurant non trouve" });
        res.json(r);
    });
    ...
    app.get("/api/orders", requireAuth, async (req, res) => {
        ...
        if (user.role === "admin") {
            if (req.query.clientId)
                filters.clientId = Number(req.query.clientId);
    ...
    app.get("/api/orders/:id", requireAuth, async (req, res) => {
        ...
        if (user?.role === "client" && order.clientId !== userId)
            return res.status(403).json({ message: "Acces refuse" });
        if (user?.role === "driver" && order.driverId !== userId)
            return res.status(403).json({ message: "Acces refuse" });
        res.json(order);
    });
    ...
    app.patch("/api/orders/:id", requireAuth, async (req, res) => {
        ...
        const currentLog = existingOrder.auditLog || [];
    ...
    app.post("/api/saved-addresses", requireAuth, async (req, res) => {
        ...
        if (!label || !address)
            return res.status(400).json({ message: "Label et adresse requis" });
        if (isDefault)
            await storage.setDefaultAddress(userId, -1);
    ...
    app.patch("/api/saved-addresses/:id", requireAuth, async (req, res) => {
        ...
        if (!existing.find(a => a.id === Number(req.params.id)))
            return res.status(403).json({ message: "Acces refuse" });
        const addr = await storage.updateSavedAddress(Number(req.params.id), req.body);
        if (!addr)
            return res.status(404).json({ message: "Adresse non trouvee" });
        res.json(addr);
    });
    ...
    app.get("/api/chat/users-by-role/:role", requireAuth, async (req, res) => {
        ...
        if (sessionUser.role === "client" && requestedRole !== "admin")
            return res.status(403).json({ message: "Acces interdit" });
        if (sessionUser.role === "driver" && requestedRole !== "admin")
            return res.status(403).json({ message: "Acces interdit" });
    ...
    app.post("/api/chat", requireAuth, async (req, res) => {
        const sessionUserId = req.session?.userId;
        if (sessionUserId !== req.body.senderId)
            return res.status(403).json({ message: "Acces interdit" });
    ...
    app.get("/api/analytics/marketing", requireAdmin, async (req, res) => {
        ...
        const onTimeDelivered = deliveredOrders.filter(o => {
            if (!o.estimatedDelivery || !o.updatedAt)
                return false;
