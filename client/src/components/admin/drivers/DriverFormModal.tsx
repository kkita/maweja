...
const defaultForm = { name: "", email: "", phone: "", password: "", vehicleType: "moto", vehiclePlate: "", driverLicense: "", commissionRate: 15 };
...
export default function DriverFormModal({ editingDriver, onClose }: Props) {
  ...
  useEffect(() => {
    if (editingDriver) {
      setForm({ name: editingDriver.name, email: editingDriver.email, phone: editingDriver.phone, password: "", vehicleType: editingDriver.vehicleType || "moto", vehiclePlate: editingDriver.vehiclePlate || "", driverLicense: editingDriver.driverLicense || "", commissionRate: editingDriver.commissionRate || 15 });
    } else {
      setForm(defaultForm);
    }
  }, [editingDriver]);
