...
import type { Restaurant, MenuItem, RestaurantCategory, BoutiqueCategory, MenuItemCategory } from "@shared/schema";
...
function AddRestaurantModal({ onClose, storeType = "restaurant" }: { onClose: () => void; storeType?: "restaurant" | "boutique" }) {
  ...
  const mutation = useMutation({
    ...
    onError: (e: any) => toast({ title: "Erreur", description: e.message || (isBoutique ? "Impossible de créer la boutique" : "Impossible de créer le restaurant"), variant: "destructive" }),
  });
...
function DeleteRestaurantModal({ restaurant, onClose, storeType = "restaurant" }: { restaurant: Restaurant; onClose: () => void; storeType?: "restaurant" | "boutique" }) {
  const isBoutique = storeType === "boutique";
  const { toast } = useToast();
  const mutation = useMutation({
...
function MenuItemForm({ restaurantId, item, onClose, storeType = "restaurant" }: { restaurantId: number; item?: MenuItem; onClose: () => void; storeType?: "restaurant" | "boutique" }) {
  const isBoutique = storeType === "boutique";
  const { toast } = useToast();
  const isEdit = !!item;
...
function MenuSection({ restaurant, storeType = "restaurant" }: { restaurant: Restaurant; storeType?: "restaurant" | "boutique" }) {
  const isBoutique = storeType === "boutique";
  const { toast } = useToast();
