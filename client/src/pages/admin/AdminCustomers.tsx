    ...
    <AdminLayout title="Gestion des clients">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Points de fidelite</p>
        </div>
      </div>

      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800">
          <h3 className="font-bold text-gray-900 dark:text-white">Liste des clients</h3>
        </div>
          ...
          {clients.map((client) => (
            <div key={client.id} className="p-5 flex items-center gap-4" data-testid={`client-row-${client.id}`}>
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">{client.email} - {client.phone}</p>
              </div>
              <div className="text-center px-4">
                <p className="font-black text-gray-900 dark:text-white">{clientOrders[client.id]?.count || 0}</p>
                <p className="text-[10px] text-gray-400">Commandes</p>
              </div>
              <div className="text-center px-4">
                <p className="font-black text-gray-900 dark:text-white">{formatPrice(clientOrders[client.id]?.total || 0)}</p>
