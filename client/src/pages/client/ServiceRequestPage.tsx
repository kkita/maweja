      ...
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <div className="max-w-lg mx-auto px-4 py-12">
          <div className="bg-white rounded-3xl border border-gray-100 p-8 text-center shadow-sm">
            ...
            <h2 className="text-xl font-black text-gray-900 mb-2">Demande envoyee !</h2>
            <p className="text-sm text-gray-500 mb-6">Notre equipe va examiner votre demande et vous contacter rapidement.</p>
            <div className="bg-gray-50 rounded-2xl p-4 mb-6 text-left">
              <p className="text-xs font-semibold text-gray-500 mb-2">Resume</p>
              <div className="space-y-1">
                <p className="text-sm text-gray-700"><strong>Service:</strong> {categoryName}</p>
                {serviceType && <p className="text-sm text-gray-700"><strong>Type:</strong> {serviceType}</p>}
                <p className="text-sm text-gray-700"><strong>Contact:</strong> {contactMethod === "whatsapp" ? "WhatsApp" : contactMethod === "phone" ? "Telephone" : "Email"}</p>
              </div>
            </div>
            <button
      ...
      <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <div className="max-w-lg mx-auto px-4 py-4">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">Demande #{existingRequest.id}</h2>
          ...
          <div className={`rounded-2xl border p-4 mb-4 ${statusColors[existingRequest.status] || statusColors.pending}`}>
            <p className="font-bold text-sm">Statut: {statusLabels[existingRequest.status] || existingRequest.status}</p>
          </div>

          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
            <div><p className="text-[10px] font-semibold text-gray-400 uppercase">Nom complet</p><p className="text-sm text-gray-900 font-medium">{existingRequest.fullName}</p></div>
            <div><p className="text-[10px] font-semibold text-gray-400 uppercase">Telephone</p><p className="text-sm text-gray-900 font-medium">{existingRequest.phone}</p></div>
            <div><p className="text-[10px] font-semibold text-gray-400 uppercase">Adresse</p><p className="text-sm text-gray-900 font-medium">{existingRequest.address}</p></div>
            {existingRequest.serviceType && <div><p className="text-[10px] font-semibold text-gray-400 uppercase">Type de service</p><p className="text-sm text-gray-900 font-medium">{existingRequest.serviceType}</p></div>}
            {existingRequest.budget && <div><p className="text-[10px] font-semibold text-gray-400 uppercase">Budget</p><p className="text-sm text-gray-900 font-medium">{existingRequest.budget}</p></div>}
            <div>
              <p className="text-[10px] font-semibold text-gray-400 uppercase">Horaire</p>
              <p className="text-sm text-gray-900 font-medium">
                {existingRequest.scheduledType === "asap" ? "Des que possible" : `${existingRequest.scheduledDate} a ${existingRequest.scheduledTime}`}
              </p>
            </div>
            {existingRequest.additionalInfo && <div><p className="text-[10px] font-semibold text-gray-400 uppercase">Informations</p><p className="text-sm text-gray-700">{existingRequest.additionalInfo}</p></div>}
            {existingRequest.adminNotes && (
              <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                <p className="text-[10px] font-bold text-blue-700 uppercase mb-1">Reponse de l'equipe</p>
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="max-w-lg mx-auto px-4 py-4">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900" data-testid="text-form-title">Nouvelle demande</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="flex gap-2 mb-3">
              <button type="button" onClick={() => setScheduledType("asap")}
                ...
                <Clock size={14} className="inline mr-1.5" />Des que possible
              </button>
              <button type="button" onClick={() => setScheduledType("scheduled")}
                ...
                <Calendar size={14} className="inline mr-1.5" />Planifier
              </button>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
            ...
            <div>
              <label className="text-[10px] font-semibold text-gray-500 uppercase mb-1 block">Nom Complet *</label>
              <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} required
                data-testid="input-fullname" placeholder="Votre nom complet"
            ...
            <div>
              <label className="text-[10px] font-semibold text-gray-500 uppercase mb-1 block">Telephone *</label>
              <div className="relative">
                <Phone size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} required
                  data-testid="input-phone" placeholder="+243 ..."
            ...
            <div>
              <label className="text-[10px] font-semibold text-gray-500 uppercase mb-1 block">Adresse *</label>
              <div className="relative">
                <MapPin size={14} className="absolute left-3 top-3 text-gray-400" />
                <input type="text" value={address} onChange={e => setAddress(e.target.value)} required
                  data-testid="input-address" placeholder="Votre adresse a Kinshasa"
                  className="w-full pl-9 pr-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-red-500 focus:outline-none" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-3">
            ...
            <div>
              <label className="text-[10px] font-semibold text-gray-500 uppercase mb-1 block">Type de {categoryName}</label>
            ...
            <div>
              <label className="text-[10px] font-semibold text-gray-500 uppercase mb-1 block">Budget estimatif</label>
              <div className="relative">
                <DollarSign size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="text" value={budget} onChange={e => setBudget(e.target.value)}
                  data-testid="input-budget" placeholder="Ex: 50000 FC"
                  className="w-full pl-9 pr-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-red-500 focus:outline-none" />
              </div>
            </div>
            <div>
              <label className="text-[10px] font-semibold text-gray-500 uppercase mb-1 block">Informations supplementaires</label>
              <textarea value={additionalInfo} onChange={e => setAdditionalInfo(e.target.value)}
                data-testid="input-additional-info" placeholder="Decrivez vos besoins en detail..."
