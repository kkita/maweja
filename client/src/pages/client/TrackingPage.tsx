    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-24">
      ...
      <div className="max-w-lg mx-auto px-4 py-4">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => navigate("/orders")} className="w-10 h-10 bg-white dark:bg-gray-900 rounded-xl flex items-center justify-center border border-gray-200 dark:border-gray-700" data-testid="button-back-orders">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6 mb-4">
          <h3 className="font-semibold text-sm text-gray-900 dark:text-white mb-6">Suivi de la commande</h3>
                ...
                <div key={step.key} className="flex gap-4" data-testid={`tracking-step-${step.key}`}>
                  <div className="flex flex-col items-center">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
                      isCurrent ? "bg-red-600 text-white shadow-lg shadow-red-200" : isCompleted ? "bg-red-100 dark:bg-red-900/40 text-red-600" : "bg-gray-100 dark:bg-gray-800 text-gray-400"
                    }`}>
                      <step.icon size={18} />
                    </div>
                    {i < steps.length - 1 && (
                      <div className={`w-0.5 h-8 my-1 ${isCompleted ? "bg-red-300" : "bg-gray-200 dark:bg-gray-700"}`} />
                    )}
                  </div>
                  <div className="pb-4">
                    <p className={`font-semibold text-sm ${isCurrent ? "text-red-600" : isCompleted ? "text-gray-900 dark:text-white" : "text-gray-400 dark:text-gray-600"}`}>
                      {step.label}
                    </p>
        ...
        {driver && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-4 mb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button className="w-10 h-10 bg-red-50 dark:bg-red-900/30 rounded-xl flex items-center justify-center text-red-600" data-testid="button-call-driver">
                <Phone size={18} />
              </button>
              <button className="w-10 h-10 bg-red-50 dark:bg-red-900/30 rounded-xl flex items-center justify-center text-red-600" data-testid="button-chat-driver">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-4 mb-4">
          <h3 className="font-semibold text-sm text-gray-900 dark:text-white mb-3">Details de la commande</h3>
          ...
          <div className="border-t border-gray-100 dark:border-gray-800 mt-3 pt-3 space-y-1">
            <div className="flex justify-between text-sm"><span className="text-gray-500 dark:text-gray-400">Livraison</span><span className="dark:text-white">{formatPrice(order.deliveryFee)}</span></div>
            <div className="flex justify-between font-bold"><span className="dark:text-white">Total</span><span className="text-red-600">{formatPrice(order.total)}</span></div>
          </div>
        </div>

        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-4">
          <h3 className="font-semibold text-sm text-gray-900 dark:text-white mb-2">Adresse de livraison</h3>
