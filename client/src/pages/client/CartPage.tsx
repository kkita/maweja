...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 pb-44" style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <ClientNav />
      <MPageHeader
        title="Mon Panier"
        ...
        action={
          <button
            onClick={clearCart}
            className="flex items-center gap-1.5 text-brand-500 font-semibold text-xs active:opacity-70 transition-opacity"
      ...
      <div className="max-w-lg mx-auto px-4 pt-4 space-y-3">
        ...
        <MCard>
          ...
          <div className="flex items-center gap-2.5 px-4 py-3.5 bg-red-50/70 dark:bg-red-950/20 border-b border-red-100/60 dark:border-red-900/30">
            <ShoppingBag size={15} className="text-brand-500 flex-shrink-0" />
            <p className="text-sm font-bold text-brand-500 flex-1 truncate" data-testid="text-restaurant-name">
              ...
              <div key={item.id} className="flex gap-3.5 p-4" data-testid={`cart-item-${item.id}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-2.5">
                    <span className="font-black text-brand-500 text-sm" data-testid={`price-${item.id}`}>
                    ...
                    <div className="flex items-center gap-0.5 bg-gray-50 dark:bg-zinc-800 rounded-[12px] p-1">
                      ...
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
                        className="w-8 h-8 rounded-[10px] bg-brand flex items-center justify-center active:scale-90 transition-transform"
          ...
          <div className="px-4 pb-4">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3 rounded-[14px] border-2 border-brand/20 text-brand-500 font-semibold text-sm active:scale-98 transition-all"
        ...
        <MCard padded>
          ...
          <textarea
            ...
            className="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700 text-gray-700 dark:text-gray-200 placeholder-gray-300 dark:placeholder-gray-600 rounded-[14px] p-3.5 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-brand-500/20"
            rows={3}
          ...
          <div className="flex items-center justify-between gap-2 mb-3">
            ...
            {defaultAddress && (
              <button
                ...
                className="flex items-center gap-0.5 text-brand-500 font-semibold"
                style={{ fontSize: 12 }}
          ...
          {defaultAddress ? (
            ...
            <div className="space-y-2">
              <input
                ...
                className="w-full bg-gray-50 dark:bg-zinc-800 border border-gray-100 dark:border-zinc-700 text-gray-700 dark:text-gray-200 placeholder-gray-300 dark:placeholder-gray-600 rounded-[14px] p-3.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20"
                data-testid="input-manual-address"
              />
              <button
                onClick={() => navigate("/addresses")}
                className="flex items-center gap-1.5 text-brand-500 font-semibold"
                style={{ fontSize: 12 }}
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center" style={{ fontSize: 13 }}>
              ...
              {!zoneResult.allowed && resolvedAddress ? (
                <span className="text-brand-500 font-bold text-xs" data-testid="text-delivery-fee">Hors zone</span>
              ) : (
            ...
            {!zoneResult.allowed && resolvedAddress && (
              <div className="bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/30 rounded-[12px] p-3" data-testid="cart-zone-warning">
                <p className="text-brand-500 text-[11px] font-bold flex items-center gap-1.5 mb-1.5">
                  <MapPin size={11} />
            ...
            <div className="border-t border-gray-100 dark:border-zinc-800 pt-3 flex justify-between items-center">
              ...
              <span className="font-black text-brand-500 text-xl" data-testid="text-grand-total">{formatPrice(grandTotal)}</span>
            </div>
          </div>
        </MCard>
      </div>
      ...
      {/* ── Sticky checkout CTA ──────────────────────────────────────── */}
      <div className="fixed bottom-[68px] left-0 right-0 px-4 py-3 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl border-t border-gray-100 dark:border-zinc-800/60 z-50">
          ...
          <button
            ...
            className={`w-full py-4 rounded-[18px] font-bold text-base flex items-center justify-center gap-2.5 transition-all active:scale-[0.98] ${
              resolvedAddress && !zoneResult.allowed
                ...
                : "text-white brand-cta-gradient brand-shadow-cta"
            }`}
          >
            {resolvedAddress && !zoneResult.allowed ? (
      ...
      {showConfirmModal && (
        ...
        >
          <div
            className="bg-white dark:bg-zinc-900 rounded-t-[28px] w-full max-w-lg p-6 pb-10"
          ...
          >
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3.5 rounded-[16px] text-white font-bold text-sm active:scale-95 transition-transform bg-brand-500 brand-shadow-md"
