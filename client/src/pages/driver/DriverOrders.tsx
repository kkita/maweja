  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
        <div>
          <p className="text-[10px] text-gray-400 font-medium">Commission livreur</p>
          <p className="text-sm font-black text-gray-900">
            {formatPrice(Math.round(order.total * 0.15))}
            <span className="text-[10px] text-gray-400 font-normal ml-1">/ {formatPrice(order.total)} total</span>
          </p>
        </div>
        <div className="text-right">
          <p className="text-[10px] text-gray-400 font-medium">Articles</p>
          <p className="text-sm font-bold text-gray-700">{order.items?.length || 0} article{(order.items?.length || 0) > 1 ? "s" : ""}</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="max-w-lg mx-auto px-4 py-5">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{orders.length} livraison{orders.length !== 1 ? "s" : ""} au total</p>
          </div>
          <button
            onClick={() => refetch()}
            className="w-9 h-9 flex items-center justify-center rounded-xl bg-white border border-gray-100 text-gray-400 hover:text-gray-600 active:scale-90 transition-all shadow-sm"
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-xs font-bold transition-all ${
                activeTab === tab.key
                  ...
                  : "text-gray-500 hover:bg-gray-50:bg-gray-800"
              }`}
            >
              {tab.label}
            </button>
