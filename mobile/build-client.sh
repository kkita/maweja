...
# Source de vérité : mobile/client/firebase/google-services.json (downloadable depuis
...
if [ -f "firebase/google-services.json" ]; then
  if [ -d "android/app" ]; then
    ...
    echo "   ✓ google-services.json → android/app/ (com.edcorp.maweja)"
  else
    echo "   ! android/app introuvable — google-services.json non copié (lancez 'npx cap add android' d'abord)"
  fi
else
  echo "   ! mobile/client/firebase/google-services.json absent — push notifications désactivées"
