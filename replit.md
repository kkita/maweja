...
- **Delivery Zones**: Three-tier zone-based delivery pricing. Zone A ($2.61): Gombe, Lingwala. Zone B ($3.70): Bandalungwa, Kinshasa, Kintambo, Barumbu, Ngiri-Ngiri, Limete 1-7, Saint Luc, Macampagne, Sakombi. Zone C ($6.09): Limete 8-16, Lemba, Ngaliema, Ozone, Pigeon, Yolo, etc. Zone detection via neighborhood name matching in `shared/deliveryZones.ts`. Out-of-zone addresses block checkout with warning. Zone badge (color-coded) shown on Cart, Checkout, Order Detail, Tracking, Admin Orders, and Driver views. `delivery_zone` column stored on orders table.
